...
import com.sparta.sweethoney.exception.order.*;
...
public class OrderService {
    ...
    public OrderCreateDto createOrder(OrderRequestDto requestDto) {
        ...
        //마감시간이 넘으면 주문 할 수 없다.
        if (isStoreClose(orderTime, store)) {
    ...
    /**
     * 전체 주문 조회 -> 유저가 OWNER || USER 따라서 다름. (미정) -> 가게 사장님도 주문 가능?
     * @param userId
     * @return
     */
    ...
    private static boolean isStoreClose(LocalDateTime orderTime, Store store) {
        return orderTime.isAfter(store.getCloseTime()) || orderTime.isBefore(store.getOpenTime());
    }
}
